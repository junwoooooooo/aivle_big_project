...
function JobList({ title, jobs, selectedJobId, onSelect, projectId, links = true }) {
  return <section className="job-center__group"><h3>{title}</h3>{jobs.length === 0 ? <p>해당 작업이 없습니다.</p> : <ul>{jobs.map((job) => <li key={job.jobId} data-status={displayStatus(job)}><button type="button" aria-pressed={selectedJobId === job.jobId} onClick={(event) => onSelect(job.jobId, event.currentTarget)}><strong>{jobTaskLabel(job.taskType, job.subjectType)}</strong><span>{STATUS_LABELS[displayStatus(job)] ?? displayStatus(job)}</span><small>{formatLocalTime(job.startedAt ?? job.updatedAt)} · {recentJobMessage(job)}</small></button>{links && <Link to={targetHref(projectId, job.targetRoute)}>이동</Link>}</li>)}</ul>}</section>;
}
