...
function JobList({ title, jobs, selectedJobId, onSelect, limit }) {
  ...
  return <section className="job-center__group"><h3>{title}</h3>{jobs.length === 0 ? <p>해당 작업이 없습니다.</p> : <><ul>{visible.map((job) => <li key={job.jobId} data-status={displayStatus(job)}><button type="button" aria-pressed={selectedJobId === job.jobId} onClick={(event) => onSelect(job.jobId, event.currentTarget)}><strong>{jobTaskLabel(job.taskType, job.subjectType)}</strong><span>{STATUS_LABELS[displayStatus(job)] ?? displayStatus(job)}</span><small>{formatLocalTime(job.startedAt ?? job.updatedAt)} · {recentJobMessage(job)}</small></button></li>)}</ul>{remaining > 0 && <p className="job-center__more">+ 외 {remaining}건</p>}</>}</section>;
}

function JobGroups({ jobs, onSelect, quick = false }) {
  ...
  return <><div className="job-center__summary" aria-label="작업 요약"><span>현재 진행 <strong>{running.length}</strong></span><span>입력 필요 <strong>{needsInput.length}</strong></span><span>최근 작업 <strong>{jobs.recent.length}</strong></span></div><div className="job-center__groups"><JobList title="현재 진행" jobs={running} limit={quick ? 1 : undefined} {...props} /><JobList title="입력 필요" jobs={needsInput} limit={quick ? 1 : undefined} {...props} /><JobList title="최근 작업" jobs={jobs.recent} limit={quick ? 3 : undefined} {...props} /></div>{!quick && <p className="job-center__basis">최근 20건 기준</p>}</>;
}
...
function JobDetail({ jobs, selectedJob, onBack, projectId, onRetryJob }) {
  ...
  return <section className="job-center__timeline" aria-live="polite"><header><div><button type="button" aria-label="전체 작업으로 돌아가기" onClick={onBack}>‹</button><h3>{selectedJob ? jobTaskLabel(selectedJob.taskType, selectedJob.subjectType) : '작업 상세'}</h3>{selectedJob && <small>{STATUS_LABELS[displayStatus(selectedJob)] ?? displayStatus(selectedJob)}</small>}</div><span>연결 {jobs.events.transport ?? '준비 중'}</span></header>{selectedJob && <dl className="job-center__current"><div><dt>현재 작업</dt><dd>{jobTaskLabel(selectedJob.taskType, selectedJob.subjectType)}</dd></div><div><dt>현재 처리</dt><dd>{latest ? jobEventMessage(latest) : recentJobMessage(selectedJob)}</dd></div><div><dt>경과 시간</dt><dd>{Math.floor(elapsedSeconds / 60)}분 {elapsedSeconds % 60}초</dd></div><div><dt>마지막 이벤트</dt><dd>{latest ? formatLocalTime(latest.occurredAt) : '-'}</dd></div></dl>}{failed && <div className="job-center__failure"><strong>작업을 완료하지 못했습니다.</strong><span>실패 시각 {formatLocalTime(selectedJob.updatedAt)}</span><span>{latest ? jobEventMessage(latest) : recentJobMessage(selectedJob)}</span><span>{selectedJob.retryable === false ? '새 실행 가능 여부를 확인해 주세요.' : '새 작업으로 다시 시도할 수 있습니다.'}</span>{canRetryHere ? <button type="button" disabled={retrying} onClick={retry}>{retrying ? '다시 시도 중' : '다시 시도'}</button> : <Link to={targetHref(projectId, selectedJob.targetRoute)}>작업 화면 열기</Link>}{validationFields.length > 0 && <details><summary>기술 정보 보기</summary><p>오류 분류: 결과 검증</p><ul>{validationFields.slice(0, 5).map((field, index) => <li key={`${field.path}-${index}`}>검증 위치: {field.path} · 기대 형식: {field.expectedType}</li>)}</ul><p>재시도 가능: {latest?.messageParams?.retryable === false ? '아니오' : '예'}</p></details>}{retryError && <span role="alert">{retryError}</span>}</div>}{selectedJob && displayStatus(selectedJob) === 'NEEDS_INPUT' && <div className="job-center__input-action"><p>필요한 정보를 해당 업무 화면에서 입력할 수 있습니다.</p><Link to={targetHref(projectId, selectedJob.targetRoute)}>작업 화면 열기</Link></div>}{selectedJob && !failed && displayStatus(selectedJob) !== 'NEEDS_INPUT' && <Link className="job-center__module-link" to={targetHref(projectId, selectedJob.targetRoute)}>작업 화면 열기</Link>}{jobs.events.error && <button type="button" onClick={jobs.events.reconnect}>연결 재시도</button>}{groupedEvents.length === 0 ? <p>수신된 상세 이벤트가 없습니다. 서버의 작업 상태를 기준으로 표시합니다.</p> : <ol>{groupedEvents.map((event) => <li key={`${event.eventId ?? event.sequence}-${event.occurredAt}`}><time>{event.groupCount > 1 ? `${formatLocalTime(event.groupStartedAt)}~${formatLocalTime(event.occurredAt)}` : formatLocalTime(event.occurredAt)}</time><div><strong>{jobEventMessage(event)}{event.groupCount > 1 ? ` × ${event.groupCount}` : ''}</strong>{traceDetailForDisplay(event) && <small>{traceDetailForDisplay(event)}</small>}{event.status === 'FAILED' && <small>실패 단계 {event.messageParams?.traceStage ?? event.stage ?? '처리'} · {event.messageParams?.retryable === false ? '재시도 불가' : '재시도 가능'}</small>}</div><span>{STATUS_LABELS[event.status] ?? event.status}</span></li>)}</ol>}</section>;
}
...
export default function JobCenter({ projectId, onTerminal, onRetryJob, refreshKey = 0, compact = false,
  ...
  return <>
    <section id="project-task-center" className={`pipeline-task-center job-center${compact ? ' job-center--compact' : ''}`} aria-labelledby="task-center-title">
      ...
      {!jobs.loading && !jobs.error && <JobGroups jobs={jobs} onSelect={select} quick={compact} />}
    ...
    {sheet?.mounted && <div className="work-center-sheet__backdrop" data-phase={sheet.phase} onMouseDown={(event) => { if (event.target === event.currentTarget) onCloseSheet(); }}>
      <section className="work-center-sheet" data-phase={sheet.phase} role="dialog" aria-modal="true" aria-labelledby="work-center-sheet-title">
        <header><div><p>WORK CENTER</p><h2 id="work-center-sheet-title">{sheet.view === 'detail' ? '작업 상세' : '프로젝트 작업 전체'}</h2></div><button type="button" aria-label="작업 센터 닫기" onClick={onCloseSheet}>×</button></header>
        <div key={sheet.view} className="work-center-sheet__content" data-direction={sheet.direction}>
          {sheet.view === 'detail' ? <JobDetail jobs={jobs} selectedJob={selectedJob} onBack={onShowList} projectId={projectId} onRetryJob={onRetryJob} />
            : <JobGroups jobs={jobs} onSelect={(jobId, trigger) => { jobs.selectJob(jobId); onOpenJob(jobId, trigger); }} />}
        </div>
      </section>
    </div>}
  </>;
