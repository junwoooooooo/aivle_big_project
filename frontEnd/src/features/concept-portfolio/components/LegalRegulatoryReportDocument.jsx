...
export default function LegalRegulatoryReportDocument({ project, selection, report }) {
  ...
  return <article className="legal-regulatory-document">
    ...
    <ReportSection number="3" title="선택 사업안 개요"><dl><div><dt>한 줄 정의</dt><dd>{concept.conceptDefinition ?? concept.summary ?? '자료 없음'}</dd></div><div><dt>주요 고객</dt><dd>{asList(concept.targetUsers).join(' · ') || '자료 없음'}</dd></div><div><dt>핵심 가치</dt><dd>{text(concept.coreValue ?? '자료 없음')}</dd></div><div><dt>제공 방식</dt><dd>{text(concept.solutionMechanism ?? '자료 없음')}</dd></div></dl></ReportSection>
    <ReportSection number="4" title="확정 사업 조건"><dl>{hypotheses.map((item) => <div key={item.hypothesisType}><dt>{HYPOTHESIS_LABELS[item.hypothesisType] ?? '사업 조건'}</dt><dd>{hypothesisDisplay(item.hypothesisType, item.finalValue ?? item.proposedValue) || '자료 없음'}</dd></div>)}</dl></ReportSection>
    <ReportSection number="5" title="필요한 조치"><List values={body.requiredControls} /></ReportSection>
    <ReportSection number="6" title="필수 고지사항"><List values={body.requiredDisclosures} /></ReportSection>
    <ReportSection number="7" title="파트너·자격·인허가"><List values={[...asList(body.partnerRequirements), ...asList(body.qualificationRequirements), ...asList(body.requiredPartnersAndQualifications)]} /></ReportSection>
    <ReportSection number="8" title="사업 구조와 역할"><dl>{[['플랫폼 역할', roles.platformRole], ['판매 주체', roles.sellerRole], ['서비스 제공 주체', roles.providerRole], ['중개 주체', roles.intermediaryRole]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || '자료 없음'}</dd></div>)}</dl></ReportSection>
    <ReportSection number="9" title="관련 법률·규제" className="legal-document__evidence"><EvidenceDocument evidence={body.officialEvidenceReferences} /></ReportSection>
    <ReportSection number="10" title="광고·표현 주의사항"><h3>사용 가능한 표현</h3><List values={advertising.allowedClaims} /><h3>반드시 함께 표시할 내용</h3><List values={advertising.requiredDisclosures} /><h3>피해야 할 표현</h3><List values={body.prohibitedVariants} /></ReportSection>
    <ReportSection number="11" title="확인되지 않은 사항"><List values={body.unknownFacts} /></ReportSection>
    <ReportSection number="12" title="거래·결제·개인정보 등 상세 검토"><h3>거래 흐름</h3><List values={body.transactionFlow} /><h3>결제·수취 흐름</h3><List values={body.paymentFlow} /><h3>개인정보 이용</h3><List values={body.personalDataUsage} /><h3>물리 활동</h3><List values={body.physicalActivities} /></ReportSection>
    <ReportSection number="13" title="변경사항 재검토 이력"><List values={delta.map((item) => item.legalReview?.safeSummary ?? item.safeSummary ?? (item.status ? legalStatusLabel(item.status) : '재검토 완료'))} empty="이번 확정 과정에서 재검토가 필요한 변경은 없었습니다." /></ReportSection>
    <ReportSection number="14" title="검토 범위와 한계"><List values={limitations} empty="본 보고서는 제공된 사업안·확정 조건과 표시된 공식 근거 범위에서 작성되었으며, 법률 자문이나 법적 보장을 의미하지 않습니다." /></ReportSection>
  </article>;
}
