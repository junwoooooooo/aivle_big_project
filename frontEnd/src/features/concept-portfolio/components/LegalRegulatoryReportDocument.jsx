...
export default function LegalRegulatoryReportDocument({ project, selection, report }) {
  ...
  return <article className="legal-regulatory-document">
    ...
    <ReportSection number="3" title="실행 체크사항" className="legal-document__execution"><div>{executionGuide.map((group) => <article key={group.title}><h3>{group.title}</h3><List values={group.values} /></article>)}</div></ReportSection>
    ...
    <ReportSection number="6" title="필요한 조치"><List values={body.requiredControls} /></ReportSection>
    <ReportSection number="7" title="필수 고지사항"><List values={body.requiredDisclosures} /></ReportSection>
    <ReportSection number="8" title="파트너·자격·인허가"><List values={[...asList(body.partnerRequirements), ...asList(body.qualificationRequirements), ...asList(body.requiredPartnersAndQualifications)]} /></ReportSection>
    ...
    <ReportSection number="11" title="광고·표현 주의사항"><h3>사용 가능한 표현</h3><List values={advertising.allowedClaims} /><h3>반드시 함께 표시할 내용</h3><List values={advertising.requiredDisclosures} /><h3>피해야 할 표현</h3><List values={body.prohibitedVariants} /></ReportSection>
    <ReportSection number="12" title="확인되지 않은 사항"><List values={body.unknownFacts} /></ReportSection>
    <ReportSection number="13" title="거래·결제·개인정보 등 상세 검토"><h3>거래 흐름</h3><List values={body.transactionFlow} /><h3>결제·수취 흐름</h3><List values={body.paymentFlow} /><h3>개인정보 이용</h3><List values={body.personalDataUsage} /><h3>물리 활동</h3><List values={body.physicalActivities} /></ReportSection>
    <ReportSection number="14" title="변경사항 재검토 이력"><List values={delta.map((item) => item.legalReview?.safeSummary ?? item.safeSummary ?? (item.status ? legalStatusLabel(item.status) : '재검토 완료'))} empty="이번 확정 과정에서 재검토가 필요한 변경은 없었습니다." /></ReportSection>
