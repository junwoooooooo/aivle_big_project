...
import { advertisingOnlyDisclosures, legalAttentionGroups, legalReportSummaryCounts, uniqueLegalItems } from '../legalReportPresentation.js';
...
export default function LegalRegulatoryReportDocument({ project, selection, report }) {
  ...
  return <article className="legal-regulatory-document">
    ...
    <ReportSection number="3" title="주요 검토 결과 요약" className="legal-document__execution"><dl><div><dt>필요한 조치</dt><dd>{summaryCounts.controls}건</dd></div><div><dt>필수 고지</dt><dd>{summaryCounts.disclosures}건</dd></div><div><dt>파트너·자격</dt><dd>{summaryCounts.partners}건</dd></div><div><dt>추가 확인</dt><dd>{summaryCounts.unknownFacts}건</dd></div></dl></ReportSection>
    ...
    <ReportSection number="11" title="광고·표현 주의사항"><h3>사용 가능한 표현</h3><List values={uniqueLegalItems(advertising.allowedClaims)} />{advertisingDisclosures.length > 0 && <><h3>광고에서 함께 표시할 내용</h3><List values={advertisingDisclosures} /></>}<h3>피해야 할 표현</h3><List values={uniqueLegalItems(body.prohibitedVariants)} /></ReportSection>
