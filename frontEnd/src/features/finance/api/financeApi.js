...
export const createFinanceApi = (client) => Object.freeze({
  ...
  template: async (projectId, options) => client.get(`${finance(projectId)}/preparation/template`, { ...options, responseType: 'blob' }),
  importDocument: async (projectId, file, options) => { const form = new FormData(); form.append('file', file); return (await client.upload(`${finance(projectId)}/preparation/import`, form, options)).data; },
