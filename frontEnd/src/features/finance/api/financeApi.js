...
const analysisOptions = (options) => ({ timeoutMs: 120000, ...options });
...
export const createFinanceApi = (client) => Object.freeze({
  ...
  template: async (projectId, options) => client.get(`${finance(projectId)}/preparation/template`, documentOptions(options)),
  importDocument: async (projectId, file, options) => { const form = new FormData(); form.append('file', file); return (await client.upload(`${finance(projectId)}/preparation/import`, form, { timeoutMs: 60000, ...options })).data; },
  ...
  analyze: async (projectId, options) => (await client.post(`${finance(projectId)}/analysis`, {}, analysisOptions(options))).data,
  currentAnalysis: async (projectId, options) => (await client.get(`${finance(projectId)}/analysis/current`, options)).data,
  analysisDocument: async (projectId, options) => client.get(`${finance(projectId)}/analysis/document`, documentOptions(options)),
