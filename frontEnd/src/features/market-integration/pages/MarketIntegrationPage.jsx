...
import PlanningChangeCard from '../components/PlanningChangeCard.jsx';
...
export default function MarketIntegrationPage() {
  ...
  return <main className="market-integration">
    ...
    {!result ? <NotConnected market={market} snapshot={snapshot} projectId={projectId} /> : <ResultView result={result} snapshot={snapshot} market={market} />}
  </main>;
}
...
function ResultView({ result, snapshot, market }) {
  ...
  return <>
    ...
    <section className="market-result-section"><h2>기획 변경 제안</h2><div className="planning-change-list">{result.planningChangeProposals.map((proposal) => <PlanningChangeCard key={proposal.proposalId} proposal={proposal} busy={market.decidingId === proposal.proposalId} onDecision={(action, modified) => market.decide(proposal.proposalId, action, modified)} />)}</div></section>
