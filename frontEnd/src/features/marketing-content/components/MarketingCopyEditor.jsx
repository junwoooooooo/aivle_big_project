...
export default function MarketingCopyEditor({ value, source, onChange, onRevisionType }) {
  ...
  return <section className="mk-panel mk-editor" aria-labelledby="mk-editor-title"><div className="mk-panel__heading"><div><p>결과 편집</p><h2 id="mk-editor-title">Copy Editor</h2></div></div>
    ...
    <label className="mk-field"><span>Headline</span><input value={value.title} maxLength="200" onChange={set('title')} /></label>
    ...
    <label className="mk-field"><span>Hashtag</span><input value={value.hashtags.join(', ')} onChange={set('hashtags',(v)=>v.split(',').map(x=>x.trim().replace(/^#/,'')).filter(Boolean).slice(0,30))} /></label>
