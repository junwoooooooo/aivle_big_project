export default function MarketingStylePanel({ value, onChange }) {
  ...
  return <section className="mk-panel" aria-labelledby="mk-style-title"><div className="mk-panel__heading"><div><p>미리보기 스타일</p><h2 id="mk-style-title">표현 방식</h2></div></div><div className="project-form-layout">
    ...
    <label className="mk-field"><span>글자 크기</span><input type="range" min="0.85" max="1.25" step="0.05" value={value.scale} onChange={set('scale')} /></label></div>
    <small>미리보기 스타일은 콘텐츠 문구의 수정 이력과 별도로 현재 화면에만 적용됩니다.</small>
  </section>;
}
