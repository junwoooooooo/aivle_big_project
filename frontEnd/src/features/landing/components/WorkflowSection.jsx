...
export default function WorkflowSection({ onNavigate }) {
  ...
  return <section id="workflow" className="landing-section landing-workflow" aria-labelledby="workflow-title"><div className="landing-container"><p className="landing-eyebrow">HOW IT WORKS</p><h2 id="workflow-title">하나의 아이디어가,<br />하나의 검증 Journey가 됩니다.</h2><p className="landing-section__lede">아이디어 입력부터 최종 보고서까지, 저장된 결과와 사용자 선택을 따라 단계적으로 진행합니다.</p><div ref={desktopRef} className="workflow-desktop" tabIndex="0" onKeyDown={onKeyDown} aria-label="5단계 사업 검증 흐름">
    <div ref={stageRef} className="workflow-stage"><div className="workflow-morph">
      <div className="workflow-copy-stack">{outgoing && <article className={`workflow-slide workflow-slide--outgoing is-${direction}`} aria-hidden="true"><p className="workflow-stage__eyebrow">현재 단계 <b>{outgoing.number}</b> / 05</p><h3>{outgoing.title}</h3><p>{outgoing.description}</p></article>}<article className={`workflow-slide workflow-slide--active${outgoing ? ` is-${direction}` : ''}`}><p className="workflow-stage__eyebrow">현재 단계 <b>{active.number}</b> / 05</p><h3>{active.title}</h3><p>{active.description}</p>{active.number === '05' && <button type="button" className="landing-text-button" onClick={() => onNavigate('demo')}>샘플 결과 보기 →</button>}</article></div>
