...
export default function WorkflowSection({ onNavigate }) {
  ...
  return <section id="workflow" className="landing-section landing-workflow" aria-labelledby="workflow-title"><div className="landing-container"><p className="landing-eyebrow">HOW IT WORKS</p><h2 id="workflow-title">하나의 아이디어가,<br />하나의 검증 파이프라인이 됩니다.</h2><p className="landing-section__lede">아이디어 입력부터 마케팅 콘텐츠 제작까지, 저장된 결과와 사용자 선택을 따라 8단계로 진행합니다.</p><div ref={desktopRef} className="workflow-desktop" tabIndex="0" onKeyDown={onKeyDown} aria-label="8단계 사업 검증 흐름">
    <div ref={stageRef} className="workflow-stage"><div className="workflow-morph">
      <div className="workflow-copy-stack">{outgoing && <article className={`workflow-slide workflow-slide--outgoing is-${direction}`} aria-hidden="true"><p className="workflow-stage__eyebrow">현재 단계 <b>{outgoing.number}</b> / 08</p><h3>{outgoing.title}</h3><p>{outgoing.description}</p></article>}<article className={`workflow-slide workflow-slide--active${outgoing ? ` is-${direction}` : ''}`}><p className="workflow-stage__eyebrow">현재 단계 <b>{active.number}</b> / 08</p><h3>{active.title}</h3><p>{active.description}</p>{active.number === '08' && <button type="button" className="landing-text-button" onClick={() => onNavigate('demo')}>샘플 결과 보기 →</button>}</article></div>
