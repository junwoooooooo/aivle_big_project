...
const money = (value) => `${formatNumber(value)} KRW`;
...
export function FinanceReadinessReportDocument({ current, projectName, embedded = false }) {
  ...
  return <article className={`launch-report-document${embedded ? ' is-embedded' : ''}`} data-report-document="finance">
    ...
    <ReportSection number="1" title="핵심 결과">
      ...
      <dl className="launch-report-document__metrics"><div><dt>손익분기점</dt><dd>{base?.breakEvenMonth ? `${base.breakEvenMonth}개월 차` : '분석 기간 내 미도달'}</dd></div><div><dt>초기 투자금 회수</dt><dd>{base?.paybackMonth ? `${base.paybackMonth}개월 차` : '분석 기간 내 미회수'}</dd></div><div><dt>필요 운전자금</dt><dd>{money(base?.requiredWorkingCapital)}</dd></div><div><dt>총 영업이익</dt><dd>{money(base?.totalOperatingProfit)}</dd></div></dl>
    </ReportSection>

    <ReportSection number="2" title="3개년 추정 손익">
      <table><thead><tr><th>구분</th>{valuesOf(result.annualProjections).map((row) => <th key={row.year}>{row.year}년 차</th>)}</tr></thead><tbody>{[
      ...
      ].map(([label, key, currency]) => <tr key={key}><th>{label}</th>{valuesOf(result.annualProjections).map((row) => <td key={row.year}>{currency ? money(row[key]) : `${formatNumber(row[key])}%`}</td>)}</tr>)}</tbody></table>
    </ReportSection>
    ...
    <ReportSection number="3" title="월별 매출·영업이익·누적 현금흐름" className="launch-report-document__monthly">
      ...
      <table><thead><tr><th>개월</th><th>매출</th><th>영업이익</th><th>누적 현금흐름</th></tr></thead><tbody>{valuesOf(result.cashFlowChart).map((row) => <tr key={row.month}><td>{row.month}</td><td>{money(row.revenue)}</td><td>{money(row.operatingProfit)}</td><td>{money(row.cumulativeCashFlow)}</td></tr>)}</tbody></table>
    </ReportSection>

    <ReportSection number="4" title="스트레스 시나리오">
      <table><thead><tr><th>시나리오</th><th>손익분기점</th><th>총 영업이익</th><th>필요 운전자금</th></tr></thead><tbody>{valuesOf(result.stressScenarios).map((row) => <tr key={row.code}><th>{row.label}</th><td>{row.breakEvenMonth ? `${row.breakEvenMonth}개월 차` : '기간 내 미도달'}</td><td>{money(row.totalOperatingProfit)}</td><td>{money(row.requiredWorkingCapital)}</td></tr>)}</tbody></table>
    </ReportSection>

    <ReportSection number="5" title="Monte Carlo 위험 분포">
      {monteCarlo ? <dl className="launch-report-document__metrics"><div><dt>시뮬레이션</dt><dd>{formatNumber(monteCarlo.simulations)}회</dd></div><div><dt>손실 확률</dt><dd>{formatNumber(monteCarlo.lossProbabilityPercent)}%</dd></div><div><dt>투자금 회수 확률</dt><dd>{formatNumber(monteCarlo.paybackProbabilityPercent)}%</dd></div><div><dt>P10 / P50 / P90</dt><dd>{money(monteCarlo.profitP10)} / {money(monteCarlo.profitP50)} / {money(monteCarlo.profitP90)}</dd></div></dl> : <p>표시할 Monte Carlo 결과가 없습니다.</p>}
    </ReportSection>
