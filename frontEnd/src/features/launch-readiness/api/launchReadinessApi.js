...
export const createLaunchReadinessApi = (client) => Object.freeze({
  ...
  professionalReport: (projectId, module) => downloadBlob(client.download(`${professional(projectId, module)}/report`, { timeoutMs: 60000 })),
  ...
  financeReport: (projectId) => downloadBlob(client.download(`${root(projectId)}/finance/analysis/report`, { timeoutMs: 60000 })),
  reports: (projectId, modules) => downloadBlob(client.download(`${root(projectId)}/reports/download?${modules.map((module) => `modules=${encodeURIComponent(module)}`).join('&')}`, { timeoutMs: 120000 })),
});
