...
function Module({ module, api, projectId, onReady, onPreview }) {
  const [result, setResult] = useState(null); const [busy, setBusy] = useState(false); const [error, setError] = useState(null); const [progress, setProgress] = useState(0); const input = useRef(null);
  useEffect(() => {
    let active = true;
  ...
  const analyze = async (event) => { const file = event.target.files?.[0]; if (!file) return; setBusy(true); setError(null); try { const value = await api.analyze(projectId, module, file); setProgress(100); setResult(value); onReady(value); } catch (e) { setError(e); } finally { event.target.value = ''; setBusy(false); } };
  const preview = () => onPreview(`${info[module][0]} 보고서 미리보기`, () => api.report(projectId, module));
  return <section className="finance-section launch-readiness-section"><div className="launch-readiness-section__heading"><div><p>{info[module][0]}</p><h2>{info[module][0]} 전문 입력 분석</h2><span>{info[module][1]}</span></div></div><div className="finance-minimal-actions"><button className="finance-save" type="button" onClick={() => void template()}>입력 템플릿 다운로드</button><label className={`finance-save ${busy ? 'is-disabled' : ''}`}><input ref={input} type="file" accept=".docx" onChange={(e) => void analyze(e)} disabled={busy} />{busy ? '분석 중…' : '분석 실행 (.docx 업로드)'}</label><button className="finance-save launch-readiness-preview" type="button" disabled={!result || busy} onClick={() => void preview()}>미리보기</button></div>{busy && <div className="launch-readiness-progress"><div><span>분석 진행 중</span><strong>{progress}%</strong></div><i><em style={{ width: `${progress}%` }} /></i><small>입력 항목을 확인하고 평가 근거와 보고서를 생성하고 있습니다. 분석이 끝날 때까지 잠시 기다려 주세요.</small></div>}{error && <p className="finance-error">{getUserErrorMessage(error)}</p>}{result && <ProfessionalConclusion module={module} result={result} />}</section>;
}
...
export default function LaunchReadinessPage() {
  ...
  const [technologyDone, setTechnologyDone] = useState(false); const [operationsDone, setOperationsDone] = useState(false); const [financeDone, setFinanceDone] = useState(false); const [selected, setSelected] = useState([]); const [downloadOpen, setDownloadOpen] = useState(false); const [error, setError] = useState(null); const [preview, setPreview] = useState(null); const financeInput = useRef(null);
  useEffect(() => () => { if (preview?.url) URL.revokeObjectURL(preview.url); }, [preview]);
  useEffect(() => { if (finance.analysis) setFinanceDone(true); }, [finance.analysis]);
  ...
  const analyzeFinance = async (event) => { const file = event.target.files?.[0]; if (!file) return; try { await finance.importAndAnalyze(file); setFinanceDone(true); } catch (e) { setError(e); } finally { event.target.value = ''; } };
  ...
  return <main className="finance-page finance-page--minimal launch-readiness-page"><header className="finance-heading"><div><p>5. 출시 준비</p><h1>출시 준비 분석</h1><span>기술·운영·재무 분석을 완료한 뒤, 맨 아래에서 필요한 보고서를 선택해 다운로드하세요.</span></div></header>{error && <p className="finance-error">{getUserErrorMessage(error)}</p>}
    ...
    <section className="finance-section launch-readiness-section"><div className="launch-readiness-section__heading"><div><p>재무 분석</p><h2>재무 입력 기반 손익·현금흐름 분석</h2><span>재무 입력 문서를 업로드해 분석합니다.</span></div></div><div className="finance-minimal-actions"><button className="finance-save" type="button" onClick={() => void finance.downloadTemplate()}>재무 템플릿 다운로드</button><label className={`finance-save ${finance.busy === 'import-analyze' ? 'is-disabled' : ''}`}><input ref={financeInput} type="file" accept=".docx" onChange={(e) => void analyzeFinance(e)} disabled={finance.busy === 'import-analyze'} />{finance.busy === 'import-analyze' ? '분석 중…' : '분석 실행 (.docx 업로드)'}</label><button className="finance-save launch-readiness-preview" type="button" disabled={!financeDone || finance.busy === 'import-analyze'} onClick={() => void openPreview('재무 분석 보고서 미리보기', () => finance.downloadFinancialAnalysisDocument())}>미리보기</button></div><FinancialConclusion analysis={finance.analysis} /></section>
