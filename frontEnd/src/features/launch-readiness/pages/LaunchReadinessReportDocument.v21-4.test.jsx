...
const professionalCurrent = (moduleType = 'LAUNCH', passed = true) => ({
  ...
  professionalInput: moduleType === 'LAUNCH'
    ? { releaseScope: '서울 지역 정식 출시', launchCriteria: '장애 대응 훈련 완료' }
...
describe('V21.4 단일 보고서 문서', () => {
  ...
  it('출시 준비 보고서는 출시 계획 DOCX 입력만 표시한다', () => {
    render(<LaunchReadinessReportDocument module="launch" current={professionalCurrent()} projectName="테스트" />);
    expect(screen.getByText('서울 지역 정식 출시')).toBeInTheDocument();
    expect(screen.getByText('장애 대응 훈련 완료')).toBeInTheDocument();
    expect(screen.queryByText('재무 분석')).not.toBeInTheDocument();
    expect(screen.queryByText('기술 분석')).not.toBeInTheDocument();
  });

  it('출시 준비 결과는 독립 결과·gate·action을 보존한다', () => {
    render(<LaunchReadinessReportDocument module="launch" current={professionalCurrent()} projectName="테스트" />);
    expect(screen.getByText('출시 전 확인 조건을 보완하면 진행할 수 있습니다.')).toBeInTheDocument();
    expect(screen.getByText('출시 기준')).toBeInTheDocument();
    expect(screen.getByText('검증 결과 확인')).toBeInTheDocument();
  });

  it('분석 결과가 없으면 성공 보고서를 지어내지 않는다', () => {
    render(<LaunchReadinessReportDocument module="launch" current={{ analysis: null }} projectName="테스트" />);
    expect(screen.getByText('표시할 출시 준비 분석 결과가 없습니다.')).toBeInTheDocument();
    expect(screen.queryByText('AI 출시 준비도 평가')).not.toBeInTheDocument();
  });

  it('보고서 page view는 출시 준비 화면으로 돌아가는 링크만 제공한다', () => {
    render(<MemoryRouter><LaunchReadinessReportPageView reportType="launch" project={{ projectId: 7, name: '테스트' }}
      documents={[{ module: 'launch', current: professionalCurrent() }]} onPrint={vi.fn()} /></MemoryRouter>);
    expect(screen.getByRole('link', { name: '출시 준비로 돌아가기' })).toHaveAttribute('href', '/app/projects/7/launch-readiness');
    expect(screen.queryByText('통합 보고서')).not.toBeInTheDocument();
  });

  it('보고서 query는 launch 단일 module만 허용한다', () => {
    expect(reportModulesFromQuery('launch', new URLSearchParams())).toEqual(['launch']);
    expect(reportModulesFromQuery('finance', new URLSearchParams())).toEqual([]);
    expect(reportModulesFromQuery('integrated', new URLSearchParams('modules=launch'))).toEqual([]);
  });

  it('출시 준비 보고서 파일명은 독립 surface 이름을 사용한다', () => {
    expect(launchReadinessReportTitle('A/B:* 서비스', 'launch', '2026-08-16T10:30:00'))
      .toBe('A_B_서비스_출시준비_보고서_20260816_1030');
  });

  it('Launch 문서는 3개 summary와 8개 의미 section, 점수 투명성·disclaimer를 보존한다', () => {
    const { container } = render(<LaunchReadinessReportDocument module="launch" current={professionalCurrent()} projectName="테스트 프로젝트" />);
  ...
  it('quality 미통과 결과를 독립 AI 검증 통과로 표시하지 않는다', () => {
    render(<LaunchReadinessReportDocument module="launch" current={professionalCurrent('LAUNCH', false)} projectName="테스트" />);
  ...
  it('legacy 통합 report query는 출시 준비 route로 해석되지 않는다', () => {
    const params = new URLSearchParams();
    params.append('modules', 'finance');
    params.append('modules', 'technology');
    params.append('modules', 'operations');
    expect(reportModulesFromQuery('integrated', params)).toEqual([]);
  });

  it('Report Page의 PDF 저장은 같은 DOM에서 window.print만 1회 호출하고 title을 복원한다', () => {
    const originalTitle = document.title;
    const print = vi.spyOn(window, 'print').mockImplementation(() => {});
    render(<MemoryRouter><LaunchReadinessReportPageView reportType="launch" project={{ projectId: 7, name: '스마트:서비스' }}
      documents={[{ module: 'launch', current: professionalCurrent() }]}
      onPrint={(completedAt) => printLaunchReadinessReport('스마트:서비스', 'launch', completedAt)} /></MemoryRouter>);
    fireEvent.click(screen.getByRole('button', { name: 'PDF로 저장' }));
    expect(print).toHaveBeenCalledTimes(1);
    expect(document.title).toBe('스마트_서비스_출시준비_보고서_20260816_0930');
    window.dispatchEvent(new Event('afterprint'));
    expect(document.title).toBe(originalTitle);
  });

  it('파일명은 Windows 금지 문자를 제거하고 출시 준비 보고서 이름을 만든다', () => {
    expect(launchReadinessReportTitle('A/B:* 서비스', 'launch', '2026-08-16T10:30:00'))
      .toBe('A_B_서비스_출시준비_보고서_20260816_1030');
  });
});
