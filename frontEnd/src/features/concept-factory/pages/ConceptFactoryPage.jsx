...
export default function ConceptFactoryPage() {
  ...
  if (!factory.run) return <section className="concept-workboard concept-workboard--empty">
    ...
    {factory.confirmedSnapshotId ? <button type="button" disabled={factory.actionPending} onClick={factory.start}>5개 컨셉 만들기</button> : <Link to={projectRoutes.idea(projectId)}>Idea Brief 확정하기</Link>}
  </section>;
