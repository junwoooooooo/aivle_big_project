...
class TechOpsHandoffTests {
    @Test
    void handoffUsesOnlyTheImmutableTechOpsInputSnapshot() {
        ProjectRepository projects=mock(ProjectRepository.class); ConceptSelectionRepository selections=mock(ConceptSelectionRepository.class);
        MarketAnalysisSeedSnapshotRepository marketSeeds=mock(MarketAnalysisSeedSnapshotRepository.class);
        ModuleHandoffRepository handoffs=mock(ModuleHandoffRepository.class); ModuleRunRepository runs=mock(ModuleRunRepository.class);
        ...
        MarketAnalysisSeedSnapshot marketSeed=mock(MarketAnalysisSeedSnapshot.class); TechOpsInputSnapshot snapshot=mock(TechOpsInputSnapshot.class);
        ...
        when(snapshot.getId()).thenReturn("tech-snapshot-1"); when(snapshot.getSnapshotHash()).thenReturn("sha256:"+"a".repeat(64));
        when(snapshot.getSnapshotJson()).thenReturn("{\"contract\":\"tech-ops-input-snapshot-v1\"}");
        ...
        assertThat(response.inputSnapshotType()).isEqualTo("TECH_OPS_INPUT");
        assertThat(response.inputSchemaVersion()).isEqualTo("2.0");
        assertThat(response.inputSnapshotId()).isEqualTo("tech-snapshot-1");
        assertThat(response.input().path("contract").asText()).isEqualTo("tech-ops-input-snapshot-v1");
    }
}
