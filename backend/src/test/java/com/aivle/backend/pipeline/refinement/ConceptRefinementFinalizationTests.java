...
class ConceptRefinementFinalizationTests {
 ...
 @BeforeEach void setup(){contract=new ConceptRefinementDecisionContract(mapper,new ConceptPortfolioJsonHasher(mapper));
  ...
  lenient().when(projects.findByIdAndOwnerIdAndDeletedAtIsNull(PROJECT,OWNER)).thenReturn(Optional.of(project));lenient().when(lineage.preApplyCurrent(eq(OWNER),eq(PROJECT),any())).thenReturn(true);
 ...
 @Test void finalizedCurrentUsesFinalSeedNotStaleSource(){ConceptRefinementRound r=appliedRound(proposal("targetUsers","old","new"),4,3);ReflectionTestUtils.setField(r,"finalMarketSeedSnapshotId","seed-new");ReflectionTestUtils.setField(r,"state",ConceptRefinementRound.State.FINALIZED);
  ConceptRefinementLineageGuard guard=mock(ConceptRefinementLineageGuard.class);when(guard.postApplyCurrent(PROJECT,r)).thenReturn(true);ConceptRefinementFinalizationService s=new ConceptRefinementFinalizationService(projects,rounds,finals,contract,guard,seeds,bmPlans,selectionService,new ConceptPortfolioJsonHasher(mapper),mapper,clock);
  ConceptRefinementFinal f=mock(ConceptRefinementFinal.class);when(f.getProjectId()).thenReturn(PROJECT);when(f.getFinalMarketSeedSnapshotId()).thenReturn("seed-new");when(f.getOutcome()).thenReturn(ConceptRefinementFinal.Outcome.REFINED);when(f.getFinalJson()).thenReturn("{}");when(finals.findTopByProjectIdAndDeletedAtIsNullOrderByCreatedAtDescIdDesc(PROJECT)).thenReturn(Optional.of(f));when(rounds.findTopByProjectIdAndDeletedAtIsNullOrderByCreatedAtDescIdDesc(PROJECT)).thenReturn(Optional.of(r));when(seeds.findByIdAndStaleAtIsNullAndDeletedAtIsNull("seed-new")).thenReturn(Optional.of(seed));
  assertThat(s.current(OWNER,PROJECT).stale()).isFalse();when(seeds.findByIdAndStaleAtIsNullAndDeletedAtIsNull("seed-new")).thenReturn(Optional.empty());assertThat(s.current(OWNER,PROJECT).stale()).isTrue();}
