...
class ConceptRefinementFinalizationTests {
 ...
 @Test void currentBindsLatestFinalToItsExactRoundInsteadOfLatestProjectRound(){ConceptRefinementRound roundA=appliedRound(proposal("targetUsers","old","new"),4,3);ConceptRefinementFinal value=finalValue(roundA,"seed-new");
  ...
  assertThat(service.current(OWNER,PROJECT).stale()).isFalse();verify(rounds).findById(101L);verify(rounds,never()).findTopByProjectIdAndDeletedAtIsNullOrderByCreatedAtDescIdDesc(PROJECT);}
 ...
 @Test void finalizingStaleReflectsPostApplyLineage(){ConceptRefinementRound r=appliedRound(proposal("targetUsers","old","new"),4,3);r.startFinalization("finalize",HASH,"task",clock.instant());when(rounds.findTopByProjectIdAndDeletedAtIsNullOrderByCreatedAtDescIdDesc(PROJECT)).thenReturn(Optional.of(r));
  var current=service.current(OWNER,PROJECT);assertThat(current.state()).isEqualTo("FINALIZING");assertThat(current.stale()).isFalse();assertThat(current.value()).isNull();when(lineage.postApplyCurrent(PROJECT,r)).thenReturn(false);assertThat(service.current(OWNER,PROJECT).stale()).isTrue();}
 ...
 @Test void noFinalAndNoRoundReturnsNotStartedWithoutStaleness(){var current=service.current(OWNER,PROJECT);assertThat(current.state()).isEqualTo("NOT_STARTED");assertThat(current.stale()).isFalse();assertThat(current.value()).isNull();}
