...
class MarketInterviewContractTests {
    ...
    @Test void participantCountBelowThreeIsRejected() {
        ObjectNode value = valid(); ((ArrayNode) value.get("participants")).remove(2);
        assertThatThrownBy(() -> MarketInterviewContract.validate(value)).isInstanceOf(ExecutionFailure.class);
    }

    @Test void participantCountAboveFiveIsRejected() {
        ObjectNode value = valid();
        for (int i = 4; i <= 6; i++) ((ArrayNode) value.get("participants")).addObject()
            .put("participantId", "P" + i).put("label", "참여자").put("profile", "프로필")
            .put("context", "맥락").putArray("needs").add("요구");
        assertThatThrownBy(() -> MarketInterviewContract.validate(value)).isInstanceOf(ExecutionFailure.class);
    }

    @Test void missingParticipantInterviewIsRejected() {
        ObjectNode value = valid(); ((ArrayNode) value.get("interviews")).remove(2);
        assertThatThrownBy(() -> MarketInterviewContract.validate(value)).isInstanceOf(ExecutionFailure.class);
    }

    @Test void duplicateParticipantInterviewIsRejected() {
        ObjectNode value = valid();
        ((ObjectNode) value.path("interviews").get(2)).put("participantId", "P2");
        assertThatThrownBy(() -> MarketInterviewContract.validate(value)).isInstanceOf(ExecutionFailure.class);
    }

    @Test void nonCanonicalParticipantIdIsRejected() {
        ObjectNode value = valid();
        ((ObjectNode) value.path("participants").get(2)).put("participantId", "CUSTOMER-3");
        ((ObjectNode) value.path("interviews").get(2)).put("participantId", "CUSTOMER-3");
        assertThatThrownBy(() -> MarketInterviewContract.validate(value)).isInstanceOf(ExecutionFailure.class);
    }
        ...
        return (ObjectNode) mapper.readTree("""
          {"contract":"market-interview-result-v1","schemaVersion":"1.0","synthetic":true,
           "participants":[
             ...
             {"participantId":"P3","label":"가상 참여자 C","profile":"실무 담당자","context":"대안 비교","needs":["지원 범위"]}],
           "interviews":[
             ...
             {"participantId":"P3","questions":[{"question":"어떤 점을 보나요?","answer":"운영 부담을 봅니다.","uncertainty":"실제 업무 확인 필요"},{"question":"무엇이 걱정되나요?","answer":"교육 시간이 걱정됩니다.","uncertainty":"현장 확인 필요"},{"question":"언제 고려하나요?","answer":"지원이 명확할 때 고려합니다.","uncertainty":"구매 맥락 확인 필요"}],"concerns":["교육"],"purchaseTriggers":["명확한 지원"],"objections":[],"unmetNeeds":[]}],
