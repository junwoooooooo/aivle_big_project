...
public class TaskRun extends BaseEntity {
 ...
 public void queueRetry(){queueRetry(LocalDateTime.now());}
 public void queueRetry(LocalDateTime eligibleAt){if((state!=TaskRunState.FAILED&&state!=TaskRunState.TIMED_OUT)||!retryable)throw new IllegalStateException("task run is not retryable");state=TaskRunState.QUEUED;retryable=false;finishedAt=null;nextAttemptAt=eligibleAt;}
