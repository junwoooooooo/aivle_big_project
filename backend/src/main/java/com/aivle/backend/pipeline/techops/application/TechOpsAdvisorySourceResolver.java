...
public class TechOpsAdvisorySourceResolver {
    ...
    public Sources resolve(Long ownerId, Long projectId) {
        ...
        ConceptPortfolioSelection selection = selections
            ...
            .orElse(null);
        ConceptPortfolioConcept concept = selection == null ? null : concepts.findByIdAndProjectIdAndDeletedAtIsNull(
            selection.getConceptId(), projectId).filter(value -> value.getRun().getId().equals(selection.getRunId()))
            .orElse(null);
        MarketAnalysisSeedSnapshot seed = snapshot.getSourceMarketSeedSnapshotId() == null ? null
            : seeds.findById(snapshot.getSourceMarketSeedSnapshotId())
                .filter(value -> projectId.equals(value.getProjectId()) && value.getDeletedAt() == null)
                .orElse(null);
        ConceptLegalRegulatoryReport legal = seed == null || seed.getLegalReportId() == null ? null
        ...
        MarketResearchRun marketRun = seed == null ? null : marketRuns
            ...
            .orElse(null);
        MarketResearchVersion market = marketRun == null ? null : marketVersions.findBySourceRunIdAndDeletedAtIsNull(marketRun.getId())
            .filter(value -> value.getKind() == MarketResearchRun.Kind.FULL
                && projectId.equals(value.getProject().getId()))
            .orElse(null);
        MarketResearchRun bmRun = market == null ? null : marketRuns
            .findTopByProjectIdAndKindAndDeletedAtIsNullOrderByCreatedAtDescIdDesc(
                projectId, MarketResearchRun.Kind.BM)
            .filter(value -> value.getState() == MarketResearchRun.State.SUCCEEDED
                && market.getId().equals(value.getSourceMarketVersionId()))
            .orElse(null);
        MarketResearchVersion bm = bmRun == null ? null : marketVersions.findBySourceRunIdAndDeletedAtIsNull(bmRun.getId())
    ...
    public boolean matches(Sources source, String snapshotId, String seedId, Long marketId,
        ...
        return source.snapshot().getId().equals(snapshotId);
    }
