...
public class TwinSurveyService {
    ...
    public TwinSurveyService(ProjectRepository projects, TwinSurveyRunRepository runs,
    ...
    private void synchronize(TwinSurveyRun run) {
        ...
        if (state == TaskRunState.FAILED || state == TaskRunState.TIMED_OUT
            ...
            if (run.getState() != TwinSurveyRun.State.FAILED) {
                log.warn("Twin survey task failed projectId={} runId={} taskRunId={} errorCode={} retryable={}",
