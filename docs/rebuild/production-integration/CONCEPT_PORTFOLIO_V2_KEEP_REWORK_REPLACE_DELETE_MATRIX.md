...
| `ai/app/api/executions.py` | 내부 AI task registry와 dispatcher | REWORK | `CONCEPT_PORTFOLIO_V2_RUN` 등록·호출 | facade result contract | 해당 없음 | P4 `CONCEPT_PORTFOLIO_V2_CONTINUE` strict dispatch까지 연결 완료 |
...
| `backend/.../taskrun/domain/TaskType.java` | task type registry | REWORK | Portfolio initial/continuation task type 포함 | P3 worker | 해당 없음 | P4 `CONCEPT_PORTFOLIO_V2_RUN`과 `CONCEPT_PORTFOLIO_V2_CONTINUE` 등록 완료 |
...
| 같은 migration의 `concept_selections`, `concept_hypothesis_decisions` | selection/hypothesis history | REWORK | 새 Portfolio Concept 참조·version history | additive FK migration | legacy Concept FK 전환 | selection row 정본 유지 |
...
| 같은 migration의 `market_analysis_seed_snapshots` | Market canonical input | REWORK | current Legal Report와 새 selection source를 반영 | additive migration | legacy source adapter 확정 | 기존 snapshot history 유지 |
