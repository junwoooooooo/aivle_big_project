...
현재 `frontEnd/src/features/projects/model/projectWorkflowModel.js`와 `projectRoutes.js`에는 이 순서가 직접 반영되어 있다. 기존 G5·G6은 법률 Boundary를 먼저 확정하고 컨셉 3개를 생성·공개하는 구조다. 이 구조는 새 제품 방향과 맞지 않고, 외부 분석 모듈을 단계 사이에 결합하기 어렵다.
