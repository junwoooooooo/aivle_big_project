...
def _search_queries(module_type: str, values: dict[str, str]) -> list[str]:
    ...
    if module_type == "TECHNOLOGY":
        return [
            f"{keywords} 공식 기술 아키텍처 보안 가이드",
            f"{keywords} 성능 테스트 장애 대응 공식 문서",
        ]
    return [
        f"{keywords} 운영 KPI SLA 파일럿 가이드",
        f"{keywords} 고객 지원 장애 대응 운영 벤치마크",
    ]
        ...
        for query in _search_queries(module_type, values):
            ...
            for item in results:
                ...
                rows.append({
                    "title": title,
                    "url": url,
                    "snippet": _plain(item.get("content"), 420),
                    "query": _plain(query, 300),
                })
